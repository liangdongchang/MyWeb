...
class TimeZoneDefinition(DYNAMIC_TIME_ZONE_INFORMATION):
    ...
    def __init_from_other(self, other):
        ...
        for name in other.field_names():
            ...
            value = super(TimeZoneDefinition, other).__getattribute__(
                other, name)
    ...
    def _locate_day(year, cutoff):
        ...
        result = datetime.datetime(year, cutoff.month, day, cutoff.hour,
                                   cutoff.minute, cutoff.second,
                                   cutoff.millisecond)
...
class TimeZoneInfo(datetime.tzinfo):
    ...
    def _get_time_zone_key(subkey=None):
        ...
        key = _RegKeyDict.open(winreg.HKEY_LOCAL_MACHINE,
...
GetTimeZoneNames = deprecated(TimeZoneInfo._get_time_zone_key_names,
                              'GetTimeZoneNames')
GetIndexedTimeZoneNames = deprecated(TimeZoneInfo._get_indexed_time_zone_keys,
                                     'GetIndexedTimeZoneNames')
GetSortedTimeZoneNames = deprecated(TimeZoneInfo.get_sorted_time_zone_names,
...
def GetTZCapabilities():
    ...
    MissingTZPatch = datetime.datetime(
        2007, 11, 2, tzinfo=tzi).utctimetuple() != (2007, 11, 2, 6, 0, 0, 4,
                                                    306, 0)
    DynamicTZSupport = not MissingTZPatch and datetime.datetime(
        2003, 11, 2, tzinfo=tzi).utctimetuple() == (2003, 11, 2, 7, 0, 0, 6,
                                                    306, 0)
    del tzi
    return vars()
