...
# $Id$
from . import constants as sspicon
...
class _BaseAuth(object):
    ...
    def encrypt(self, data):
        ...
        encbuf.append(
            win32security.PySecBufferType(trailersize,
    ...
    def sign(self, data):
        ...
        sigbuf.append(
            win32security.PySecBufferType(len(data), sspicon.SECBUFFER_DATA))
        sigbuf.append(
            win32security.PySecBufferType(sigsize, sspicon.SECBUFFER_TOKEN))
...
class ClientAuth(_BaseAuth):
    ...
    def authorize(self, sec_buffer_in):
        ...
        err, attr, exp = win32security.InitializeSecurityContext(
            self.credentials, ctxtin, self.targetspn, self.scflags,
            self.datarep, sec_buffer_in, self.ctxt, sec_buffer_out)
...
class ServerAuth(_BaseAuth):
    ...
    def __init__(self,
        ...
        self.credentials, self.credentials_expiry = win32security.AcquireCredentialsHandle(
            spn, self.pkg_info['Name'], sspicon.SECPKG_CRED_INBOUND, None,
            None)
        _BaseAuth.__init__(self)
