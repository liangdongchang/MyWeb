...
class DialogDef:
    ...
    def createDialogTemplate(self):
        t = None
        self.template = [[
            self.caption, (self.x, self.y, self.w, self.h), self.style,
            self.styleEx, (self.fontSize, self.font)
        ]]
        # Add the controls
...
class ControlDef:
    ...
    def createDialogTemplate(self):
        ...
        if ct in _controlMap:
            ct = _controlMap[ct]
        t = [
            ct, self.label, self.idNum, (self.x, self.y, self.w, self.h),
...
class RCParser:
    ...
    def lang(self):
        while self.token[0:
                         4] == "LANG" or self.token[0:
                                                    7] == "SUBLANG" or self.token == ',':
            self.getToken()
    ...
    def parse_dialog(self, name):
        ...
        self.getToken()
        while not (self.token is None or self.token == "" or
                   self.token == "END"):
            if self.token == "STYLE":
    ...
    def styles(self, defaults, defaultStyle):
        ...
        i = 0
        Not = False
        while ((i % 2 == 1 and ("|" == self.token or "NOT" == self.token)) or
               (i % 2 == 0)) and not self.token is None:
            Not = False
            if "NOT" == self.token:
                Not = True
                self.getToken()
...
if __name__ == '__main__':
    if len(sys.argv) <= 1:
        ...
        print()
        print(
            "See test_win32rcparser.py, and the win32rcparser directory (both")
