...
import win32.api as win32api
import win32.constants as win32con
import winerror
import win32.eventlog as win32evtlog
...
def SafeFormatMessage(eventLogRecord, logType=None):
    ...
    try:
        ...
        return "<The description for Event ID ( %d ) in Source ( %r ) could not be found. It contains the following insertion string(s):%r.>" % (
            ...
            eventLogRecord.SourceName, desc)


def FeedEventLogRecords(feeder,
                        machineName=None,
                        logName="Application",
                        readFlags=None):
