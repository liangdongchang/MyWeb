...
import win32.com
from win32.com.client import gencache

com_error = win32.com.com_error
_univgw = win32.com._univgw
...
def RegisterInterfaces(typelibGUID, lcid, major, minor, interface_names=None):
    ...
    try:
        ...
        mod = None
    if mod is None:
        import win32.com.client.build
        # Load up the typelib and build (but don't cache) it now
        tlb = win32.com.LoadRegTypeLib(typelibGUID, major, minor, lcid)
        ...
        if interface_names is None:
            ...
            for i in range(tlb.GetTypeInfoCount()):
                ...
                if attr.typekind == win32.com.TKIND_INTERFACE or (
                        attr.typekind == win32.com.TKIND_DISPATCH and
                        attr.wTypeFlags & win32.com.TYPEFLAG_FDUAL):
...
def _CalcTypeSize(typeTuple):
    t = typeTuple[0]
    if t & (win32.com.VT_BYREF | win32.com.VT_ARRAY):
        # Its a pointer.
        cb = _univgw.SizeOfVT(win32.com.VT_PTR)[1]
    elif t == win32.com.VT_RECORD:
...
class Method:
    ...
    def _GenerateInArgTuple(self):
        ...
        for arg in self.args:
            if arg.inOut & win32.com.PARAMFLAG_FIN or \
                    arg.inOut == 0:
    ...
    def _GenerateOutArgTuple(self):
        ...
        l = []
        for arg in self.args:
            if arg.inOut & win32.com.PARAMFLAG_FOUT or \
               arg.inOut & win32.com.PARAMFLAG_FRETVAL or \
               arg.inOut == 0:
...
class Definition:
    ...
    def dispatch(self,
        ...
        # Like win32.com, we special case a tuple.
