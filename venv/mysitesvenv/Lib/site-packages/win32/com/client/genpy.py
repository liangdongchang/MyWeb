...
# been translated.
mapVTToTypeString = {
    win32.com.VT_I2: 'types.IntType',
    win32.com.VT_I4: 'types.IntType',
    win32.com.VT_R4: 'types.FloatType',
    win32.com.VT_R8: 'types.FloatType',
    win32.com.VT_BSTR: 'types.StringType',
    win32.com.VT_BOOL: 'types.IntType',
    win32.com.VT_VARIANT: 'types.TypeType',
    win32.com.VT_I1: 'types.IntType',
    win32.com.VT_UI1: 'types.IntType',
    win32.com.VT_UI2: 'types.IntType',
    win32.com.VT_UI4: 'types.IntType',
    win32.com.VT_I8: 'types.LongType',
    win32.com.VT_UI8: 'types.LongType',
    win32.com.VT_INT: 'types.IntType',
    win32.com.VT_DATE: 'win32.com.PyTimeType',
    win32.com.VT_UINT: 'types.IntType',
}
...
def WriteSinkEventMap(obj, stream):
    ...
    for name, entry in list(obj.propMapGet.items()) + \
        ...
        print(
            '\t\t%9d : "%s",' % (entry.desc[0],
                                 MakeEventMethodName(entry.names[0])),
            file=stream)
    print('\t\t}', file=stream)
        ...
        if self.aliasDoc:
            ...
            if isinstance(ai, type(0)):
                try:
                    ...
                    print(
                        self.doc[0] + " = None # Can't convert alias info " +
                        str(ai),
                        file=stream)
        print(file=stream)
        self.bWritten = 1
...
class EnumerationItem(build.OleItem, WritableItem):
    ...
    def __init__(self, typeinfo, attr, doc=None, bForUser=1):
        ...
        self.mapVars = {}
        typeFlags = attr[11]
        self.hidden = typeFlags & win32.com.TYPEFLAG_FHIDDEN or \
            typeFlags & win32.com.TYPEFLAG_FRESTRICTED
...
##    WriteAliasesForItem(self, aliasItems)
    ...
    def WriteEnumerationItems(self, stream):
        ...
        for name in names:
            ...
            vdesc = entry.desc
            if vdesc[4] == win32.com.VAR_CONST:
                val = vdesc[1]
    ...
    def WriteVTableMap(self, generator):
        ...
        for v in self.vtableFuncs:
            ...
            for arg in arg_desc:
                ...
                print(
                    repr((arg[0], arg[1], defval, arg3_repr)),
                    ",",
                    end=' ',
                    file=stream)
            print("],", end=' ', file=stream)
...
class DispatchItem(build.DispatchItem, WritableItem):
    ...
    def WriteClass(self, generator):
        if not self.bIsDispatch and not self.type_attr.typekind == win32.com.TKIND_DISPATCH:
            return
    ...
    def WriteClassHeader(self, generator):
        ...
        stream = generator.file
        print(
            'class ' + self.python_name + '(DispatchBaseClass):', file=stream)
        if doc[1]:
            print('\t' + build._makeDocString(doc[1]), file=stream)
        try:
            progId = win32.com.ProgIDFromCLSID(self.clsid)
            print(
                "\t# This class is creatable by the name '%s'" % (progId),
                file=stream)
    ...
    def WriteEventSinkClassHeader(self, generator):
        ...
        print('\t\t\timport win32.com.server.util', file=stream)
        print(
            '\t\t\tfrom win32.com.server.policy import EventHandlerPolicy',
            file=stream)
        print(
            '\t\t\tcpc=oobj._oleobj_.QueryInterface(win32.com.IID_IConnectionPointContainer)',
            file=stream)
        print('\t\t\tcp=cpc.FindConnectionPoint(self.CLSID_Sink)', file=stream)
        print(
            '\t\t\tcookie=cp.Advise(win32.com.server.util.wrap(self, usePolicy=EventHandlerPolicy))',
            file=stream)
        ...
        print('\t\texcept win32.com.com_error:', file=stream)
        ...
        print('\t\timport win32.com.server.util', file=stream)
        print(
            '\t\tif iid==self.CLSID_Sink: return win32.com.server.util.wrap(self)',
            file=stream)
        print(file=stream)
        self.bWritten = 1
    ...
    def WriteCallbackClassBody(self, generator):
        ...
        print(
            ...
            file=stream)
        for name, entry in list(self.propMapGet.items()) + list(
                self.propMapPut.items()) + list(self.mapFuncs.items()):
            fdesc = entry.desc
            methName = MakeEventMethodName(entry.names[0])
            print(
                '#\tdef ' + methName + '(self' + build.BuildCallList(
                    fdesc,
                    entry.names,
                    "defaultNamedOptArg",
            ...
            if entry.doc and entry.doc[1]:
                print(
                    '#\t\t' + build._makeDocString(entry.doc[1]), file=stream)
        print(file=stream)
        self.bWritten = 1
    ...
    def WriteClassBody(self, generator):
        ...
        for name in names:
            ...
            dispid = entry.desc[0]
            if entry.desc[9] & win32.com.FUNCFLAG_FRESTRICTED and \
                    dispid != win32.com.DISPID_NEWENUM:
                continue
            # If not accessible via IDispatch, then we can't use it here.
            if entry.desc[3] != win32.com.FUNC_DISPATCH:
                continue
            if dispid == win32.com.DISPID_VALUE:
                lkey = "value"
            elif dispid == win32.com.DISPID_NEWENUM:
            ...
            if generator.bBuildHidden or not entry.hidden:
                if entry.GetResultName():
                    print(
                        '\t# Result is of type ' + entry.GetResultName(),
                        file=stream)
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                mapEntry = MakeMapLineEntry(details[0],
                                            win32.com.DISPATCH_PROPERTYGET,
                                            resultDesc, argDesc, key,
                                            entry.GetResultCLSIDStr())

                if entry.desc[0] == win32.com.DISPID_VALUE:
                    lkey = "value"
                elif entry.desc[0] == win32.com.DISPID_NEWENUM:
                    lkey = "_newenum"
                else:
                    lkey = key.lower()
                if lkey in specialItems and specialItems[lkey] is None:  # remember if a special one.
                    specialItems[lkey] = (
                        entry, win32.com.DISPATCH_PROPERTYGET, mapEntry)
                    # All special methods, except _newenum, are written
                    # "normally".  This is a mess!
                    if entry.desc[0] == win32.com.DISPID_NEWENUM:
                        continue

                print(
                    '\t\t"%s": %s,' % (build.MakePublicAttributeName(key),
                                       mapEntry),
                    file=stream)
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                print(
                    ...
                    (build.MakePublicAttributeName(key), details[0],
                     win32.com.DISPATCH_PROPERTYPUT, defArgDesc),
                    file=stream)
        ...
        if specialItems["value"]:
            ...
            print(
                "\t# Default %s for this class is '%s'" % (typename,
                                                           entry.names[0]),
                file=stream)
            for line in ret:
                print(line, file=stream)
            if sys.version_info > (3, 0):
                ...
                print("\t\ttry:", file=stream)
                print(
                    "\t\t\treturn unicode(self.__call__(*args))", file=stream)
                print("\t\texcept win32.com.com_error:", file=stream)
        ...
        if specialItems["_newenum"]:
            ...
            invkind = win32.com.DISPATCH_METHOD | win32.com.DISPATCH_PROPERTYGET
            resultCLSID = "None"
        ...
        print(
            ...
            (win32.com.DISPID_NEWENUM, invkind),
            file=stream)
        print('\t\texcept win32.com.error:', file=stream)
        print(
        ...
        print(
            '\t\treturn win32.com.client.util.Iterator(ob, %s)' % resultCLSID,
            file=stream)
    ...
    def WriteClass(self, generator):
        ...
        if generator.generate_type == GEN_DEMAND_CHILD:
            ...
            for ref in referenced_items:
                print(
                    "__import__('%s.%s')" % (generator.base_mod_name,
                                             ref.python_name),
                    file=stream)
                print(
                    "%s = sys.modules['%s.%s'].%s" %
        ...
        try:
            progId = win32.com.ProgIDFromCLSID(self.clsid)
            print(
                "# This CoClass is known by the name '%s'" % (progId),
                file=stream)
        except win32.com.com_error:
            pass
        print(
            'class %s(CoClassBaseClass): # A CoClass' % (self.python_name),
            file=stream)
        if doc and doc[1]:
        ...
        for item, flag in self.sources:
            if flag & win32.com.IMPLTYPEFLAG_FDEFAULT:
                defItem = item
        ...
        for item, flag in self.interfaces:
            if flag & win32.com.IMPLTYPEFLAG_FDEFAULT:  # and dual:
                defItem = item
...
class Generator:
    ...
    def _Build_CoClass(self, type_info_tuple):
        ...
        for j in range(attr[8]):
            ...
            child_infos.append((info, refAttr.typekind, refType,
                                refType.GetDocumentation(-1), refAttr, flags))

        # Done generating children - now the CoClass itself.
        newItem = CoClassItem(info, attr, doc)
        return newItem, child_infos

    def _Build_CoClassChildren(self, coclass, coclass_info, oleItems,
                               vtableItems):
        sources = {}
        interfaces = {}
        for info, info_type, refType, doc, refAttr, flags in coclass_info:
            #          sys.stderr.write("Attr typeflags for coclass referenced object %s=%d (%d), typekind=%d\n" % (name, refAttr.wTypeFlags, refAttr.wTypeFlags & win32.com.TYPEFLAG_FDUAL,refAttr.typekind))
            if refAttr.typekind == win32.com.TKIND_DISPATCH or (
                    refAttr.typekind == win32.com.TKIND_INTERFACE and
                    refAttr[11] & win32.com.TYPEFLAG_FDISPATCHABLE):
                clsid = refAttr[0]
                ...
                if flags & win32.com.IMPLTYPEFLAG_FSOURCE:
                    dispItem.bIsSink = 1
                    sources[dispItem.clsid] = (dispItem, flags)
                else:
                    interfaces[dispItem.clsid] = (dispItem, flags)
                # If dual interface, make do that too.
                if clsid not in vtableItems and refAttr[11] & win32.com.TYPEFLAG_FDUAL:
                    refType = refType.GetRefTypeInfo(
                        refType.GetRefTypeOfImplType(-1))
                    refAttr = refType.GetTypeAttr()
                    assert refAttr.typekind == win32.com.TKIND_INTERFACE, "must be interface bynow!"
    ...
    def _Build_Interface(self, type_info_tuple):
        ...
        if infotype == win32.com.TKIND_DISPATCH or (
                infotype == win32.com.TKIND_INTERFACE and
                attr[11] & win32.com.TYPEFLAG_FDISPATCHABLE):
            oleItem = DispatchItem(info, attr, doc)
            # If this DISPATCH interface dual, then build that too.
            if (attr.wTypeFlags & win32.com.TYPEFLAG_FDUAL):
                # Get the vtable interface
                refhtype = info.GetRefTypeOfImplType(-1)
                info = info.GetRefTypeInfo(refhtype)
                attr = info.GetTypeAttr()
                infotype = win32.com.TKIND_INTERFACE
            else:
                infotype = None
        assert infotype in [None, win32.com.TKIND_INTERFACE
                            ], "Must be a real interface at this point"
        if infotype == win32.com.TKIND_INTERFACE:
    ...
    def BuildOleItemsFromType(self):
        ...
        for type_info_tuple in self.CollectOleItemInfosFromType():
            info, infotype, doc, attr = type_info_tuple
            clsid = attr[0]
            if infotype == win32.com.TKIND_ENUM or infotype == win32.com.TKIND_MODULE:
            ...
            # hidden, assuming that you only ever use them via the CoClass)
                    ...
                    win32.com.TKIND_DISPATCH, win32.com.TKIND_INTERFACE
            ]:
                if clsid not in oleItems:
                    ...
                    if vtableItem is not None:
                        vtableItems[clsid] = vtableItem
            elif infotype == win32.com.TKIND_RECORD or infotype == win32.com.TKIND_UNION:
                newItem = RecordItem(info, attr, doc)
                recordItems[newItem.clsid] = newItem
            elif infotype == win32.com.TKIND_ALIAS:
                # We dont care about alias' - handled intrinsicly.
                continue
            elif infotype == win32.com.TKIND_COCLASS:
                newItem, child_infos = self._Build_CoClass(type_info_tuple)
                self._Build_CoClassChildren(newItem, child_infos, oleItems,
                                            vtableItems)
    ...
    def do_gen_file_header(self):
        ...
        print(
            '# Created by makepy.py version %s' % (makepy_version, ),
            file=self.file)
        print(
        ...
        print(
            ...
            file=self.file)
        print('import win32.com.client.util', file=self.file)
        print('from win32types import IID', file=self.file)
        print('from win32.com.client import Dispatch', file=self.file)
        print(file=self.file)
        print(
            '# The following 3 lines may need tweaking for the particular server',
            file=self.file)
        print(
            '# Candidates are win32.com.Missing, .Empty and .ArgNotFound',
            file=self.file)
        print('defaultNamedOptArg=win32.com.Empty', file=self.file)
        print('defaultNamedNotOptArg=win32.com.Empty', file=self.file)
        print('defaultUnnamedArg=win32.com.Empty', file=self.file)
        print(file=self.file)
    ...
    def do_generate(self):
        ...
        for record in recordItems.values():
            if record.clsid == win32.com.IID_NULL:
                print(
        ...
        if enumItems:
            print(
                'win32.com.client.constants.__dicts__.append(constants.__dict__)',
                file=stream)
        print(file=stream)
    ...
    def generate_child(self, child, dir):
        ...
        major = la[3]
        minor = la[4]
        self.base_mod_name = "win32.com.gen_py." + \
        ...
        try:
            ...
            for type_info_tuple in infos:
                info, infotype, doc, attr = type_info_tuple
                if infotype == win32.com.TKIND_COCLASS:
            ...
            if not found:
                ...
                for type_info_tuple in infos:
                    info, infotype, doc, attr = type_info_tuple
                    if infotype in [
                            win32.com.TKIND_INTERFACE, win32.com.TKIND_DISPATCH
                    ]:
                        if build.MakePublicAttributeName(doc[0]) == child:
                            found = 1
                            oleItem, vtableItem = self._Build_Interface(
    ...
    def do_gen_child_item(self, oleitem):
        ...
        if oleitem.bWritten:
            print(
                'win32.com.client.CLSIDToClass.RegisterCLSID( "%s", %s )' %
    ...
    def checkWriteDispatchBaseClass(self):
        if not self.bHaveWrittenDispatchBaseClass:
            print(
                "from win32.com.client import DispatchBaseClass",
                file=self.file)
    ...
    def checkWriteCoClassBaseClass(self):
        if not self.bHaveWrittenCoClassBaseClass:
            print(
                "from win32.com.client import CoClassBaseClass",
                file=self.file)
...
if __name__ == '__main__':
    print(
        "This is a worker module.  Please use makepy to generate Python files.")
