...
from win32 import com as pythoncom
from win32 import types as pywintypes
...
def Dispatch(dispatch,
    ...
    return __WrapDispatch(
        dispatch, userName, resultCLSID, typeinfo, clsctx=clsctx)
...
def DispatchWithEvents(clsid, user_event_class):
    ...
    * The user_event_class as passed to this function.
    ...
    result_class = new_type("COMEventClass", (disp_class, events_class,
                                              user_event_class),
...
class DispatchBaseClass:
        ...
        if oobj is None:
            ...
            try:
                oobj = oobj._oleobj_.QueryInterface(
                    self.CLSID,
                    pythoncom.IID_IDispatch)  # Must be a valid COM instance
    ...
    def __repr__(self):
        ...
        return "<%s.%s instance at 0x%s>" % (mod_name, self.__class__.__name__,
                                             id(self))
...
def _get_good_object_(obj, obUserName=None, resultCLSID=None):
    if obj is None:
        ...
        return tuple(
            map(_get_good_object_, obj, obUserNameTuple, resultCLSIDTuple))
    else:
        return _get_good_single_object_(obj, obUserName, resultCLSID)
...
class VARIANT(object):
    ...
    def __repr__(self):
        return "win32com.client.VARIANT(%r, %r)" % (self.varianttype,
                                                    self._value)
